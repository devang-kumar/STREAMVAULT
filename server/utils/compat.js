...
/**
 * Normalize CMS publication status for Series/Season/Episode/Movie.
 * Maps legacy series values (upcoming/ongoing/completed) to CMS values.
 */
export const normalizePublicationStatus = (status, fallback = 'draft') => {
  const s = String(status || fallback).toLowerCase().trim();
  if (['draft', 'published', 'archived'].includes(s)) return s;
  if (s === 'upcoming') return 'draft';
  if (s === 'ongoing') return 'published';
