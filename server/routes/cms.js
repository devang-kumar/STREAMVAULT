...
import { transformSeries, transformEpisode, transformMovie } from '../utils/transform.js';
...
router.post('/series', upload.fields([
  ...
  try {
    ...
    let posterUrl = req.body.poster_url || req.body.thumbnail_url || '';
    let logoUrl = req.body.logo_url || '';
    let trailerUrl = req.body.trailer_url || '';
    const media = {
      posterPublicId: req.body.poster_public_id || req.body.thumbnail_public_id || '',
      bannerPublicId: req.body.banner_public_id || ''
    };

    if (req.files?.banner) {
      const result = await uploadToCloudinary(req.files.banner[0].buffer, 'streamvault/series/banners', 'image');
      bannerUrl = result.secure_url;
      media.bannerPublicId = result.public_id;
    }
    if (req.files?.thumbnail) {
      const result = await uploadToCloudinary(req.files.thumbnail[0].buffer, 'streamvault/series/thumbnails', 'image');
      posterUrl = result.secure_url;
      media.posterPublicId = result.public_id;
    }
    if (req.files?.logo) {
      const result = await uploadToCloudinary(req.files.logo[0].buffer, 'streamvault/series/logos', 'image');
...
router.patch('/series/:id', upload.fields([
  ...
  try {
    ...
    if (req.body.logo_url !== undefined) updateData.logoUrl = req.body.logo_url;
    ...
    if (req.files?.banner) {
      ...
      updateData['media.bannerPublicId'] = result.public_id;
    }
    if (req.body.banner_url) updateData.banner = req.body.banner_url;
    if (req.body.banner_public_id !== undefined) updateData['media.bannerPublicId'] = req.body.banner_public_id;
    if (req.body.poster_url) {
      updateData.poster = req.body.poster_url;
      updateData.thumbnail = req.body.poster_url;
    }
    if (req.body.poster_public_id !== undefined) updateData['media.posterPublicId'] = req.body.poster_public_id;
    if (req.body.thumbnail_public_id !== undefined) updateData['media.posterPublicId'] = req.body.thumbnail_public_id;
    if (req.files?.thumbnail) {
      const result = await uploadToCloudinary(req.files.thumbnail[0].buffer, 'streamvault/series/thumbnails', 'image');
      updateData.poster = result.secure_url;
      updateData.thumbnail = result.secure_url;
      updateData['media.posterPublicId'] = result.public_id;
    }
    if (req.body.thumbnail_url) {
      updateData.poster = req.body.thumbnail_url;
      updateData.thumbnail = req.body.thumbnail_url;
    }
    if (req.files?.logo) {
      const result = await uploadToCloudinary(req.files.logo[0].buffer, 'streamvault/series/logos', 'image');
...
router.delete('/series/:id', async (req, res) => {
  try {
    ...
    if (series.media?.posterPublicId) await deleteFromCloudinary(series.media.posterPublicId, 'image').catch(() => {});
    if (series.media?.bannerPublicId) await deleteFromCloudinary(series.media.bannerPublicId, 'image').catch(() => {});
...
router.post('/series/:seriesId/seasons', upload.fields([
  { name: 'thumbnail', maxCount: 1 },
  { name: 'banner', maxCount: 1 }
]), async (req, res) => {
  try {
    ...
    const { title, release_year, description, poster_url, order, status, is_premium } = req.body;
    if (!title) return res.status(400).json({ message: 'Title is required' });
    ...
    if (req.files?.thumbnail) {
      const result = await uploadToCloudinary(req.files.thumbnail[0].buffer, 'streamvault/seasons/thumbnails', 'image');
      thumbnailUrl = result.secure_url;
      if (!posterUrl) posterUrl = thumbnailUrl;
    }
    if (req.files?.banner) {
      const result = await uploadToCloudinary(req.files.banner[0].buffer, 'streamvault/seasons/banners', 'image');
      bannerUrl = result.secure_url;
    }

    const seasonStatus = normalizePublicationStatus(status, 'draft');
...
router.get('/series/:seriesId/seasons/:seasonId', async (req, res) => {
  try {
    if (!validateIdParam(req, res, 'seasonId')) return;
...
router.patch('/series/:seriesId/seasons/:seasonId', upload.fields([
  { name: 'thumbnail', maxCount: 1 },
  { name: 'banner', maxCount: 1 }
]), async (req, res) => {
  try {
    console.log('PATCH season req.params:', req.params);
    if (!validateIdParam(req, res, 'seasonId')) return;
    ...
    if (req.body.thumbnail_url !== undefined) updateData.thumbnailUrl = req.body.thumbnail_url;
    if (req.body.banner_url !== undefined) updateData.bannerUrl = req.body.banner_url;
    ...
    if (req.files?.thumbnail) {
      ...
      if (!updateData.poster) updateData.poster = result.secure_url;
    }
    if (req.files?.banner) {
      const result = await uploadToCloudinary(req.files.banner[0].buffer, 'streamvault/seasons/banners', 'image');
...
router.post('/series/:seriesId/seasons/:seasonId/episodes', upload.fields([
  ...
  try {
    ...
    const { title, episode_number, duration_minutes, description, status, is_premium } = req.body;
    if (!title) return res.status(400).json({ message: 'Title is required' });

    let videoUrl = req.body.video_url || '';
    let thumbnailUrl = req.body.thumbnail_url || '';
    let previewVideoUrl = req.body.preview_video_url || req.body.previewVideoUrl || '';
    let videoPublicId = req.body.video_public_id || '';
    let thumbnailPublicId = req.body.thumbnail_public_id || '';
    let previewVideoPublicId = req.body.preview_video_public_id || '';
    ...
    const episodeStatus = normalizePublicationStatus(status, 'published');
    const episode = await Episode.create({
      ...
      video: { url: videoUrl, publicId: videoPublicId, duration: parseDuration(duration_minutes) },
...
router.get('/episodes/:episodeId', async (req, res) => {
  try {
    if (!validateIdParam(req, res, 'episodeId')) return;
...
router.patch('/episodes/:episodeId', upload.fields([
  ...
  try {
    console.log('PATCH episode req.params:', req.params);
    if (!validateIdParam(req, res, 'episodeId')) return;
    ...
    if (req.body.preview_video_url !== undefined) updateData.previewVideoUrl = req.body.preview_video_url;
    if (req.body.previewVideoUrl !== undefined) updateData.previewVideoUrl = req.body.previewVideoUrl;
    if (req.body.preview_video_public_id !== undefined) updateData.previewVideoPublicId = req.body.preview_video_public_id;
    if (req.body.thumbnail_public_id !== undefined) updateData.thumbnailPublicId = req.body.thumbnail_public_id;
    if (req.body.video_public_id !== undefined) updateData['video.publicId'] = req.body.video_public_id;
    if (req.body.order !== undefined) updateData.order = toNum(req.body.order);
    if (req.body.is_premium !== undefined) updateData.premium = toBool(req.body.is_premium);
...
router.delete('/episodes/:episodeId', async (req, res) => {
  try {
    ...
    if (episode.thumbnailPublicId) {
      await deleteFromCloudinary(episode.thumbnailPublicId, 'image').catch(() => {});
    }
    if (episode.previewVideoPublicId) {
      await deleteFromCloudinary(episode.previewVideoPublicId, 'video').catch(() => {});
    }
...
router.get('/movies', async (req, res) => {
  try {
    ...
    return res.json({ data: items.map(transformMovie), total, page: Number(page), limit: Number(limit), totalPages: Math.ceil(total / Number(limit)) });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
});
...
router.get('/movies/:id', async (req, res) => {
  try {
    ...
    return res.json({ data: transformMovie(movie) });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
});
...
router.post('/movies', upload.fields([
  ...
  try {
    ...
    let bannerUrl = req.body.banner_url || '';
    let thumbnailUrl = req.body.poster_url || req.body.thumbnail_url || '';
    let videoUrl = req.body.video_url || '';
    let bannerPublicId = req.body.banner_public_id || '';
    let posterPublicId = req.body.poster_public_id || req.body.thumbnail_public_id || '';
    let videoPublicId = req.body.video_public_id || '';
    ...
    flushCache();
    return res.status(201).json({ data: transformMovie(movie) });
  } catch (error) {
...
router.patch('/movies/:id', upload.fields([
  ...
  try {
    ...
    if (req.body.is_premium !== undefined) {
      updateData.premium = toBool(req.body.is_premium);
    ...
    if (req.body.banner_public_id !== undefined) updateData.bannerPublicId = req.body.banner_public_id;
    if (req.body.poster_public_id !== undefined) updateData.posterPublicId = req.body.poster_public_id;
    if (req.body.thumbnail_public_id !== undefined) updateData.posterPublicId = req.body.thumbnail_public_id;
    if (req.body.video_public_id !== undefined) updateData['video.publicId'] = req.body.video_public_id;
    if (req.body.thumbnail_url) {
      updateData.poster = req.body.thumbnail_url;
      updateData.thumbnail = req.body.thumbnail_url;
    ...
    flushCache();
    return res.json({ data: transformMovie(updated) });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
});
...
router.delete('/movies/:id', async (req, res) => {
  try {
    ...
    if (movie.posterPublicId) await deleteFromCloudinary(movie.posterPublicId, 'image').catch(() => {});
    if (movie.bannerPublicId) await deleteFromCloudinary(movie.bannerPublicId, 'image').catch(() => {});
