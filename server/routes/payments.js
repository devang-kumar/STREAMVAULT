...
router.post('/verify', protect, async (req, res) => {
  try {
    ...
    try {
      const wasPreviouslyPremium = false; // This is a new payment
      if (wasPreviouslyPremium) {
        await NotificationService.notifySubscriptionRenewed(req.user._id, planDoc?.name || 'Premium');
      } else {
        await NotificationService.notifySubscriptionActivated(req.user._id, planDoc?.name || 'Premium');
      }
      await NotificationService.notifyPaymentSuccess(req.user._id, {
        planId,
        planName: planDoc?.name || planId,
        amount: amount
      });
