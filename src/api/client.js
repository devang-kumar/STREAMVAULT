...
export async function getMovies(filters = {}) {
  ...
  const query = params.toString() ? `?${params.toString()}` : ''
  return request(`/movies${query}`, { method: 'GET' })
}
...
export async function getContinueWatching() {
  return request('/users/continue-watching', { method: 'GET' }, true)
}
...
export async function getStatus() {
  return request(`/status?t=${Date.now()}`, { method: 'GET', cache: 'no-store' })
}
...
export async function getNotifications(params = {}) {
  ...
  if (params.unreadOnly) qs.set('unreadOnly', params.unreadOnly)
  const query = qs.toString() ? `?${qs.toString()}` : ''
  return request(`/notifications${query}`, { method: 'GET' }, true)
}

export async function getUnreadNotificationCount() {
  return request('/notifications/unread-count', { method: 'GET' }, true)
}

export async function markNotificationRead(id) {
  return request(`/notifications/${id}/read`, { method: 'PATCH' }, true)
}

export async function markAllNotificationsRead() {
  return request('/notifications/read-all', { method: 'PATCH' }, true)
}

export async function deleteNotification(id) {
  return request(`/notifications/${id}`, { method: 'DELETE' }, true)
}

export async function clearAllNotifications() {
  return request('/notifications', { method: 'DELETE' }, true)
}
...
export async function getNotificationHistory(params = {}) {
  ...
  const query = qs.toString() ? `?${qs.toString()}` : ''
  return request(`/notifications/history${query}`, { method: 'GET' }, true)
}
...
export async function adminGetNotificationStats() {
  return request('/notifications/admin/stats', { method: 'GET' }, true)
}

export async function adminGetAllNotifications(params = {}) {
  const qs = new URLSearchParams()
  if (params.page) qs.set('page', params.page)
  if (params.limit) qs.set('limit', params.limit)
  if (params.type) qs.set('type', params.type)
  ...
  const query = qs.toString() ? `?${qs.toString()}` : ''
  return request(`/notifications/admin/all${query}`, { method: 'GET' }, true)
}

// ─── Watchlist ───────────────────────────────────────────────────────
export async function getWatchlist() {
  return request('/users/watchlist', { method: 'GET' }, true)
}

export async function addToWatchlist(showId, contentType = 'series') {
  return request('/users/watchlist', {
    method: 'POST',
    body: JSON.stringify({ showId, contentType })
  }, true)
}

export async function removeFromWatchlist(showId, contentType = 'series') {
  return request(`/users/watchlist/${showId}?contentType=${contentType}`, { method: 'DELETE' }, true)
}
