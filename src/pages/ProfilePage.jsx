...
import { getMe, getContinueWatching, updateContinueWatching, changeEmail, changePassword, getSubscription, cancelSubscription, updatePlan, updateUserProfile } from '../api/client'
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-[#1E1E2E] bg-[#0c0c14] p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${sub?.isActive ? 'bg-[#D4A017]/20 text-[#D4A017]' : 'bg-gray-700 text-gray-400'}`}>
      ...
      <div className="space-y-2">
        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider px-1">Select a Plan to Switch</p>
        {[
      ...
      {sub?.isActive && sub?.plan !== 'Basic' && (
        <button onClick={handleCancel} disabled={loading} className="w-full rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-400 transition-all hover:bg-red-500/20 disabled:opacity-50">
...
function ChangeEmailModal({ profile, onClose, onRefresh }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        <label className="block text-xs text-gray-400 mb-1.5 font-medium">Current Password</label>
        <div className="relative">
          <Lock size={14} className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-gray-500" />
          <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className="input-dark input-dark--icon-left input-dark--icon-right text-sm py-2.5" placeholder="Enter current password" required />
      ...
      <div>
        <label className="block text-xs text-gray-400 mb-1.5 font-medium">New Email</label>
        <div className="relative">
          <Mail size={14} className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-gray-500" />
          <input type="email" value={newEmail} onChange={e => setNewEmail(e.target.value)} className="input-dark input-dark--icon-left text-sm py-2.5" placeholder="new@email.com" required />
        </div>
      </div>
      <div>
        <label className="block text-xs text-gray-400 mb-1.5 font-medium">Confirm New Email</label>
        <div className="relative">
          <Mail size={14} className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-gray-500" />
          <input type="email" value={confirmEmail} onChange={e => setConfirmEmail(e.target.value)} className="input-dark input-dark--icon-left text-sm py-2.5" placeholder="new@email.com" required />
        </div>
      </div>
      <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#D4A017] px-4 py-3 text-sm font-semibold text-black transition-all hover:bg-[#b8860b] disabled:opacity-50">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        ...
        <div className="relative">
          <Lock size={14} className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-gray-500" />
          <input type={showCurrent ? 'text' : 'password'} value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} className="input-dark input-dark--icon-left input-dark--icon-right text-sm py-2.5" placeholder="Enter current password" required />
      ...
      <div>
        ...
        <div className="relative">
          <Lock size={14} className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-gray-500" />
          <input type={showNew ? 'text' : 'password'} value={newPassword} onChange={e => setNewPassword(e.target.value)} className="input-dark input-dark--icon-left input-dark--icon-right text-sm py-2.5" placeholder="Min 6 characters" required minLength={6} />
      ...
      <div>
        ...
        <div className="relative">
          <Lock size={14} className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-gray-500" />
          <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="input-dark input-dark--icon-left text-sm py-2.5" placeholder="Re-enter new password" required minLength={6} />
        </div>
      </div>
      <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#D4A017] px-4 py-3 text-sm font-semibold text-black transition-all hover:bg-[#b8860b] disabled:opacity-50">
...
function EditProfileModal({ profile, onClose, onRefresh }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="relative">
          <img
            ...
            alt="Preview"
            className="h-24 w-24 rounded-2xl border-2 border-[#D4A017] object-cover bg-[#0F0F1A]"
          />
        </div>
        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Avatar Preview</p>
      </div>
      ...
      <div>
        <label className="block text-xs text-gray-400 mb-1.5 font-medium">Choose an Avatar Template</label>
      ...
      <div>
        <label className="block text-xs text-gray-400 mb-1.5 font-medium">Or type a Custom Avatar Seed</label>
        <input
          type="text"
          value={avatarSeed}
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#D4A017] px-4 py-3 text-sm font-semibold text-black transition-all hover:bg-[#b8860b] disabled:opacity-50"
      >
...
export default function ProfilePage() {
  ...
  const [navigationStack, setNavigationStack] = useState(['settings'])

  const fetchContinueWatching = useCallback(async () => {
    try {
      const data = await getContinueWatching()
      setContinueList(Array.isArray(data) ? data : [])
    } catch (_) {
      setContinueList([])
    }
  }, [])
  ...
  // Open subscription modal directly when navigated from premium gate (e.g. WatchPage Upgrade button)
  useEffect(() => {
    if (location.state?.openSubscription) {
      setSettingsOpen(true)
      setActiveModal('subscription')
      setNavigationStack(['settings', 'subscription'])
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      if (e.key === 'Escape') {
        if (activeModal) {
          // Child screen: go back
          handleBack()
        } else {
          // Root screen: close modal
          handleCloseModal()
        }
      }
    }
  ...
  const settingsRows = [
    { label: 'Profile Details', value: profile?.name || '-', icon: Settings, color: '#ec4899', onClick: () => { setActiveModal('edit-profile'); setNavigationStack(['settings', 'edit-profile']); } },
    { label: 'Subscription', value: profile?.plan || 'Free', icon: Crown, color: '#D4A017', onClick: () => { setActiveModal('subscription'); setNavigationStack(['settings', 'subscription']); } },
    { label: 'Email', value: profile?.email || '-', icon: Mail, color: '#3b82f6', onClick: () => { setActiveModal('email'); setNavigationStack(['settings', 'email']); } },
    { label: 'Password', value: '••••••••', icon: Lock, color: '#22c55e', onClick: () => { setActiveModal('password'); setNavigationStack(['settings', 'password']); } },
    { label: 'Parental Controls', value: 'Off', icon: Shield, color: '#a78bfa', onClick: () => { setActiveModal('parental'); setNavigationStack(['settings', 'parental']); } },
  ]
  ...
  const handleBack = () => {
    if (navigationStack.length > 1) {
      ...
      setActiveModal(newStack.length > 1 ? newStack[newStack.length - 1] : null)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] px-4 pb-16 pt-24 sm:px-6">
          ...
          <div className="glass rounded-2xl p-4 sm:p-6">
            ...
            <div className="space-y-3">
              {continueList.length === 0 ? (
                <p className="text-sm text-gray-500">No content in progress. Start watching something!</p>
              ) : (
                continueList.map((item) => (
                  <div
                    key={`${item.showId}-${item.episodeId}`}
                    onClick={() => {
                      navigate(`/watch/${item.showId}/${item.episodeId}`)
                    }}
                  ...
                  >
                    <div className="relative h-14 w-24 flex-shrink-0 overflow-hidden rounded-lg bg-[#111118]">
                      <img
                        src={item.poster || `https://api.dicebear.com/9.x/identicon/svg?seed=${item.showId}`}
                        alt={item.showTitle}
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-white">{item.showTitle}</p>
                      <p className="truncate text-xs text-gray-500">
                        S{item.seasonNumber}E{item.episodeNumber} • {item.episodeTitle}
                      </p>
                      <p className="text-[10px] text-gray-600 mt-0.5">{item.progress}% complete</p>
                    </div>
                    <ChevronRight size={14} className="text-gray-600 group-hover:text-white flex-shrink-0" />
                  </div>
                ))
              )}
            </div>
          ...
          <div className="glass mt-6 rounded-2xl p-4 sm:p-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                ...
                className="w-full rounded-lg bg-[#D4A017]/10 px-4 py-2 text-sm text-[#D4A017] transition-colors hover:bg-[#D4A017]/20 sm:w-auto"
              >
                Manage
              </button>
            </div>
      ...
      <div className={`fixed inset-0 z-50 transition-opacity duration-300 ${settingsOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}>
        <button className="absolute inset-0 bg-black/60" onClick={handleCloseModal} aria-label="Close settings" />
        <aside className={`glass absolute left-1/2 top-1/2 max-h-[90vh] w-[92%] max-w-[420px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-[#1E1E2E] bg-[#09090e] p-5 transition-all duration-300 ${settingsOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
          <div className="mb-5 flex items-center justify-between rounded-xl border border-[#1E1E2E] bg-[#0c0c14] p-3">
            <div className="flex items-center gap-3 min-w-0">
              <img src={avatarUrl} alt={profile?.name} className="h-12 w-12 rounded-lg border border-[#D4A017]/50" />
              <div className="min-w-0">
                <p className="truncate text-base font-semibold text-white">{profile?.name}</p>
                <p className="truncate text-xs text-gray-500">{profile?.email}</p>
              </div>
            </div>
            {/* Root screen: show [X] button to close modal */}
            {!activeModal ? (
              <button onClick={handleCloseModal} className="rounded p-1 text-gray-400 hover:text-white" aria-label="Close panel">
                <X size={16} />
              </button>
            ) : (
              /* Child screen: show Back button */
              <button onClick={handleBack} className="flex items-center gap-1 text-[11px] text-gray-500 hover:text-white transition-colors">
          ...
          <div className="mb-4 flex flex-col items-center rounded-xl border border-[#1E1E2E] bg-black/40 p-4 text-center">
            <img src={avatarUrl} alt={profile?.name} className="mb-2 h-24 w-24 rounded-full border border-[#D4A017]/40 object-cover" />
            <p className="text-2xl font-semibold text-[#D4A017]">{profile?.name}</p>
          ...
          {!activeModal ? (
            <div className="space-y-2">
              {settingsRows.map((row) => (
                <button key={row.label} onClick={row.onClick} className="flex w-full items-center justify-between rounded-md border border-[#2a2a3d] bg-[#0e0e16] px-3 py-3 text-left transition-colors hover:border-[#D4A017]/50">
                  <div className="flex items-center gap-2">
                    <row.icon size={14} style={{ color: row.color }} />
                    <span className="text-sm text-gray-300">{row.label}</span>
                  </div>
                  <span className="flex items-center gap-1 text-sm text-gray-400">
                    {row.value} <ChevronRight size={14} />
                  </span>
                </button>
              ))}
            </div>
            ...
            <div>
              <h3 className="text-base font-semibold text-white mb-4">{modalTitle}</h3>
              {activeModal === 'edit-profile' && <EditProfileModal profile={profile} onClose={handleBack} onRefresh={refreshProfile} />}
              {activeModal === 'subscription' && <SubscriptionModal profile={profile} onClose={handleBack} onRefresh={refreshProfile} />}
              {activeModal === 'email' && <ChangeEmailModal profile={profile} onClose={handleBack} onRefresh={refreshProfile} />}
              {activeModal === 'password' && <ChangePasswordModal onClose={handleBack} />}
              {activeModal === 'parental' && (
                <div className="text-center py-6">
                  <Shield size={32} className="mx-auto text-[#a78bfa] mb-3" />
                  <p className="text-base font-semibold text-white mb-1">Parental Controls</p>
                  <p className="text-sm text-gray-500">Coming soon. This feature is under development.</p>
                  <button onClick={handleBack} className="mt-4 rounded-lg bg-[#D4A017]/10 px-4 py-2 text-sm font-semibold text-[#D4A017] transition-colors hover:bg-[#D4A017]/20">Back</button>
                </div>
              )}
            </div>
          )}
