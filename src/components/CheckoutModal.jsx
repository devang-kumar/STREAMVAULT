...
export default function CheckoutModal({ plan, onClose }) {
  ...
  const amount = plan.price.monthly   // single source of truth — all prices are flat per plan
  const hasTrial = !!plan.trialNote
  ...
  const billingPeriod = billingPeriodMap[plan.id] || 'month'
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      const { orderId, currency, amount: orderAmount, key } = await createPaymentOrder(plan.id, amount * 100)
