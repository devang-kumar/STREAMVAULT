...
export default function MovieForm() {
  ...
  const handleThumbnailUpload = async (file) => {
    try {
      const res = await adminUploadImage(file, 'streamvault/movies/thumbnails')
  ...
  const handleBannerUpload = async (file) => {
    try {
      const res = await adminUploadImage(file, 'streamvault/movies/banners')
      const url = res.data?.url || ''
  ...
  const handleVideoUpload = async (file) => {
    try {
      const res = await adminUploadVideo(file, 'streamvault/movies/videos')
      const url = res.data?.url || ''
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (form.banner_url) fd.append('banner_url', form.banner_url)
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#0F0F10] px-3 py-2">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-xs"><span className="text-white">Premium Movie</span> <span className="text-[#9CA3AF]">(only premium users can watch)</span></span>
            </label>
          </div>
