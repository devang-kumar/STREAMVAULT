...
export default function HeroBanner() {
  ...
  return (
    <section className="relative h-[72vh] min-h-[420px] overflow-hidden sm:h-[78vh] md:min-h-[500px] lg:h-[84vh]">
      ...
      <div className={`absolute inset-0 flex items-end px-4 pb-14 pt-20 transition-all duration-500 sm:px-6 sm:pb-16 lg:items-center lg:px-16 ${animating ? 'translate-y-4 opacity-0' : 'translate-y-0 opacity-100'}`}>
        <div className="max-w-2xl">
          <div className="mb-3 flex flex-wrap items-center gap-2 sm:mb-4 sm:gap-3">
            {show.tag && <span className="rounded-full bg-[#D4A017] px-3 py-1 text-[10px] font-bold text-white sm:text-xs">{show.tag}</span>}
            <div className="flex items-center gap-1 text-[#F5C518]">
              <Star size={12} className="fill-[#F5C518]" />
              <span className="text-xs font-medium text-gray-300 sm:text-sm">{show.rating}</span>
            </div>
          </div>
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-gray-300 sm:mb-4 sm:gap-3 sm:text-sm">
            <span>{show.year}</span>
          ...
          <p className="mb-6 max-w-lg line-clamp-3 text-sm leading-relaxed text-gray-200 sm:text-base">{show.description}</p>

          <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:gap-3">
            <button
              onClick={() => navigate(`/series/${show.id}`)}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-black transition-all hover:bg-white/90 sm:px-6 sm:py-3"
            >
      ...
      {featured.length > 1 && (
        <>
          <div className="absolute bottom-6 left-4 flex items-center gap-2 sm:left-6 lg:bottom-8 lg:left-16">
            {featured.map((_, i) => (
              <button
                key={i}
                onClick={() => slideTo(i)}
                className={`h-1 rounded-full transition-all duration-300 ${i === current ? 'w-8 bg-[#D4A017]' : 'w-4 bg-white/30 hover:bg-white/50'}`}
                aria-label={`Go to slide ${i + 1}`}
              />
            ))}
          </div>

          <div className="absolute bottom-4 right-3 hidden items-center gap-1 sm:flex lg:bottom-5 lg:right-4">
